...
@bot.tree.command(name="debug_generaux", description="Diagnostic des généraux et rôles de pays")
async def debug_generaux(interaction: discord.Interaction):
    ...
    # Fonction de correspondance améliorée (identique à celle de /mes_generaux)
    def test_country_match(role_name, stored_pays):
        """Teste si un rôle correspond à un pays stocké - Logique identique à /mes_generaux."""
        import re
    ...
    # Afficher les rôles de pays avec plus de détails
    if user_country_roles:
        ...
        embed.add_field(
            ...
            value="\n\n".join(roles_info[:3]),  # Limiter pour éviter embed trop long
            inline=False
        )
        
        if len(user_country_roles) > 3:
            embed.add_field(
                name="ℹ️ Rôles supplémentaires",
                value=f"... et {len(user_country_roles) - 3} autres rôles de pays",
                inline=False
            )
    else:
        embed.add_field(
    ...
    # Afficher les généraux de l'utilisateur avec tests de correspondance détaillés
    if user_id in generaux_data and "generaux" in generaux_data[user_id]:
        generaux_info = []
        ...
        for nom, data in generaux_data[user_id]["generaux"].items():
            ...
            generaux_info.append(f"  Pays stocké: `{pays_general}`")
            generaux_info.append(f"  Domaine: {domaine}")
            
            # Tester la correspondance avec CHAQUE rôle en détail
            match_found = False
            for role in user_country_roles:
                match, reason = test_country_match(role.name, pays_general)
                if match:
                    correspondances_info.append(f"✅ **{nom}** ↔ **{role.name}**")
                    correspondances_info.append(f"   Raison: {reason}")
                    match_found = True
                    break
                else:
                    # Afficher les tentatives qui ont échoué pour debug
                    correspondances_info.append(f"❌ {nom} vs {role.name}: {reason}")
            
            if not match_found:
                correspondances_info.append(f"🚫 **{nom}** - Aucune correspondance avec vos rôles actuels")
            
            correspondances_info.append("")  # Ligne vide pour séparer
        
        # Utiliser plusieurs fields pour éviter la limite de caractères
        if generaux_info:
            embed.add_field(
                name=f"⚔️ Vos généraux ({len(list(generaux_data[user_id]['generaux'].keys()))})",
                value="\n".join(generaux_info[:10]),  # Limiter pour éviter embed trop long
                inline=False
            )
        
        if correspondances_info:
            # Diviser les correspondances en chunks plus petits
            correspondances_text = "\n".join(correspondances_info[:15])
            if len(correspondances_text) > 1024:  # Limite Discord pour un field
                correspondances_text = correspondances_text[:1020] + "..."
            
            embed.add_field(
                name="🔄 Tests de correspondance",
                value=correspondances_text,
                inline=False
            )
    else:
        embed.add_field(
            name="⚔️ Vos généraux",
            value="❌ Aucun général trouvé dans vos données",
            inline=False
        )
    ...
    embed.add_field(
        ...
        value=f"• User ID: `{user_id}`\n"
              f"• Généraux dans fichier: {'✅' if user_id in generaux_data else '❌'}\n"
              f"• Section généraux: {'✅' if user_id in generaux_data and 'generaux' in generaux_data[user_id] else '❌'}",
        inline=False
    )
    ...
    embed.add_field(
        name="💡 Solution",
        value="Si aucune correspondance n'est trouvée, les noms des pays dans vos généraux "
              "ne correspondent pas à vos rôles actuels. Contactez un admin pour corriger les données.",
        inline=False
    )
...
# === COMMANDE DE TEST EN DIRECT ===

@bot.tree.command(name="test_correspondance", description="Test en direct de la logique de correspondance pays-rôles")
@app_commands.describe(
    role_name="Nom du rôle à tester (optionnel)",
    pays_stocke="Nom du pays stocké à tester (optionnel)"
)
async def test_correspondance(interaction: discord.Interaction, role_name: str = None, pays_stocke: str = None):
    """Test en direct de la fonction de correspondance entre rôles et pays."""
    ...
    # Fonction de test EXACTE de celle utilisée dans /mes_generaux
    def test_country_match_debug(role_name, stored_pays):
        """Version debug de la fonction de correspondance."""
        import re
        ...
        debug_info = []
        debug_info.append(f"🔍 **Test de correspondance détaillé**")
        debug_info.append(f"Rôle utilisateur: `{role_name}` → `{user_country}`")
        debug_info.append(f"Pays stocké: `{stored_pays}` → `{pays_general}`")
        debug_info.append("")
        
        # Test 1: Vérification directe
        debug_info.append(f"**Test 1 - Correspondance exacte:**")
        exact_match = (pays_general == user_country)
        debug_info.append(f"'{pays_general}' == '{user_country}' → {'✅ OUI' if exact_match else '❌ NON'}")
        
        if exact_match:
            return True, "Correspondance exacte", debug_info
        
        # Test 2: Vérification par inclusion
        debug_info.append(f"**Test 2 - Correspondance par inclusion:**")
        pays_in_role = pays_general in user_country
        role_in_pays = user_country in pays_general
        debug_info.append(f"'{pays_general}' in '{user_country}' → {'✅ OUI' if pays_in_role else '❌ NON'}")
        debug_info.append(f"'{user_country}' in '{pays_general}' → {'✅ OUI' if role_in_pays else '❌ NON'}")
        
        if pays_in_role or role_in_pays:
            return True, "Correspondance par inclusion", debug_info
        
        # Test 3: Vérification après nettoyage
        debug_info.append(f"**Test 3 - Correspondance après nettoyage:**")
        pays_clean = re.sub(r'[^\w\s]', '', pays_general).strip()
        role_clean = re.sub(r'[^\w\s]', '', user_country).strip()
        debug_info.append(f"Pays nettoyé: `{stored_pays}` → `{pays_clean}`")
        debug_info.append(f"Rôle nettoyé: `{role_name}` → `{role_clean}`")
        
        if pays_clean and role_clean:
            clean_exact = pays_clean == role_clean
            clean_pays_in_role = pays_clean in role_clean
            clean_role_in_pays = role_clean in pays_clean
            
            debug_info.append(f"'{pays_clean}' == '{role_clean}' → {'✅ OUI' if clean_exact else '❌ NON'}")
            debug_info.append(f"'{pays_clean}' in '{role_clean}' → {'✅ OUI' if clean_pays_in_role else '❌ NON'}")
            debug_info.append(f"'{role_clean}' in '{pays_clean}' → {'✅ OUI' if clean_role_in_pays else '❌ NON'}")
            
            if clean_exact or clean_pays_in_role or clean_role_in_pays:
                return True, "Correspondance après nettoyage", debug_info
        else:
            debug_info.append("❌ Chaînes vides après nettoyage")
        
        debug_info.append("")
        debug_info.append("**🚫 RÉSULTAT FINAL: AUCUNE CORRESPONDANCE**")
        
        return False, "Aucune correspondance", debug_info
    
    embed = discord.Embed(
        title="🧪 Test de Correspondance",
        color=EMBED_COLOR
    )
    
    # Si des paramètres sont fournis, les tester
    if role_name and pays_stocke:
        match, reason, debug_info = test_country_match_debug(role_name, pays_stocke)
        
        embed.description = f"Test manuel: `{role_name}` ↔ `{pays_stocke}`"
        embed.add_field(
            name="🎯 Résultat",
            value=f"{'✅ CORRESPONDANCE' if match else '❌ AUCUNE CORRESPONDANCE'}\nRaison: {reason}",
            inline=False
        )
        
        # Diviser les infos de debug en plusieurs fields si nécessaire
        debug_text = "\n".join(debug_info)
        if len(debug_text) <= 1024:
            embed.add_field(name="🔍 Détails", value=debug_text, inline=False)
        else:
            # Diviser en chunks
            chunks = [debug_info[i:i+15] for i in range(0, len(debug_info), 15)]
            for i, chunk in enumerate(chunks[:3]):  # Max 3 chunks
                embed.add_field(
                    name=f"🔍 Détails {i+1}/{len(chunks)}" if len(chunks) > 1 else "🔍 Détails",
                    value="\n".join(chunk),
                    inline=False
                )
    
    # Sinon, tester avec les données réelles de l'utilisateur
    else:
        # Obtenir les rôles de pays de l'utilisateur
        def get_user_country_roles(user):
            country_roles = []
        ...
        if not user_country_roles:
            embed.description = "❌ Vous n'avez aucun rôle de pays détecté."
            embed.add_field(
                name="💡 Usage manuel",
                value="Utilisez: `/test_correspondance role_name:Votre_Rôle pays_stocke:Pays_Général`",
                inline=False
            )
        elif user_id not in generaux_data or "generaux" not in generaux_data[user_id]:
            embed.description = "❌ Vous n'avez aucun général dans les données."
            embed.add_field(
                name="💡 Usage manuel",
                value="Utilisez: `/test_correspondance role_name:Votre_Rôle pays_stocke:Pays_Général`",
                inline=False
            ...
            # Tester automatiquement toutes les combinaisons
            embed.description = f"Test automatique avec vos données réelles"
            
            test_results = []
            for role in user_country_roles[:3]:  # Limiter à 3 rôles
                for nom_general, data in list(generaux_data[user_id]["generaux"].items())[:3]:  # Limiter à 3 généraux
                    pays_general = data.get("pays", "INCONNU")
                    match, reason, debug_info = test_country_match_debug(role.name, pays_general)
                    
                    result_icon = "✅" if match else "❌"
                    test_results.append(f"{result_icon} **{role.name}** ↔ **{nom_general}** ({pays_general})")
                    test_results.append(f"   Résultat: {reason}")
            
            if test_results:
                embed.add_field(
                    name="🔄 Tests automatiques",
                    value="\n".join(test_results[:10]),  # Limiter à 10 lignes
                    inline=False
                )
            
            embed.add_field(
                name="💡 Test manuel détaillé",
                value="Pour voir tous les détails d'un test spécifique:\n"
                      "`/test_correspondance role_name:Nom_Exact_Rôle pays_stocke:Nom_Exact_Pays`",
                inline=False
            )
